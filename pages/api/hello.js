  ...
  for(var a = 0; a < newnegup.length; a++){
      ...
      for(var x = 0; x<newpostup.length; x++) {
          ...
          if ((newpostup[x][1]) < Math.abs((newnegup[a][1])) && (newpostup[x][1]) > 0) {
              ...
              //log payment
              console.log(newpostup[x][0], " pay ", newnegup[a][0], ": ", newpostup[x][1])
              transactions.push(newpostup[x][0] + " pay " + newnegup[a][0] + ": " + newpostup[x][1])
              // reset the fams pay
              newpostup[x][1] = 0
          }
      }
      ...
      if (newpostup[y][1] > 0) {
          ...
          for(var z = 0; z<newnegup.length; z++) {
              ...
              if (Math.abs(newnegup[z][1]) > 0) {
                  ...
                  console.log(newpostup[y][0], ' pay ', newnegup[z][0], ": ", Math.abs(newnegup[z][1]))
                  transactions.push(newpostup[y][0] + ' pay ' +  newnegup[z][0] + ": " + Math.abs(newnegup[z][1]))
